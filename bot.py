...
ketchummapsbot = Bot(command_prefix="!")
...
@ketchummapsbot.command(pass_context=True)
async def ketchummapsbot(ctx, channel):
    ...
    await ketchummapsbot.add_roles(ctx.message.author, role)
    await ketchummapsbot.add_reaction(ctx.message, u"\U0001F44B")
...
async def working(ctx, *, status : str):
    ...
    return await ketchummapsbot.change_presence(game=discord.Game(name=status))
